...
# x는 반드시 곱해야 하기 때문에 문자형은 들어오면 안된다. 
# 만약에 함수 이름만 봤을 때, x에 뭘 넣어야 하지? 
# 만약에 숨겨져 있다면? 그래서 힌트라는게 있다.
# 위와 같이 같이 입력을 해 놓으면, 인풋과 아웃풋의 값이 명확하기 때문에 좀 더 가독성도 좋아진다.

print(func_mul3(5.0))

# 이 함수를 사용할 때는 인트형을 여기다 넣어야 하는구나.
# 예외를 발생시키지는 않지만, 이 함수는 리스트를 반환한다는 것을 코드 상에서 알려주는 것. 
# 나중에 패키지를 배우면 이 함수를 분리해서 다른 파일로 놔두면, 이 함수에 대한 설명을 보면 이런 내용들이 나타나게 된다.
# 그래서 힌트를 사용하는 것을 알고 있어야 한다. 함수를 만들 때 이걸 명시적으로 입출력값이 뭐다 하고 알려줄 수 있따.

# 람다식 예제
# 람다식을 쓰는 이유는? 메모리 절약도 되고. 가독성 향상도 되고. 코드도 간결해지고. 
# 여러 장점도 있지만, 공식 레퍼런스에서 사용하는 내용은 위와 같다.
# 함수는 객체 생성 -> 리소스(메모리)를 할당하는데,
# 람다는 즉시 실행한다. (Heap 초기화) -> 메모리가 초기화
...
var_func = mul_10
print(var_func)
# <function mul_10 at 0x000001CD08CEA950>
# 함수를 사용하지는 않았지만, 메모리에 할당이 되어 있음. 
# 함수의 객체가 생성어서 var에 func에 할당을 해 놓은 것이고. 
# 이것에 대한 타입을 찍어보면
print(type(var_func))

# class 형태로 function 이라는 클래스로 생성이 되었음. 메모리를 차지하고 있는 것
# 실행할 때는 이 자체가 함수이기 떄문에 이름이 바뀌었다.

print(var_func(10))
# 실행할 때는 이 자체가 함수이기 때문에 이름이 바뀌었다. 
# 프린트 문에 이렇게 출력을 하면 값이 100이 나온다.
# 이게 일반적으로 파이썬 인터프리터 내에서의 과정

# 정리하면
# 함수도 객체라서 파이썬에서 리소스와 메모리가 할당되고. 
# 반면에 람다는 즉시 실행되기 때문에, 메모리를 조금 더 효율적으로 사용할 수 있음
# 중요한 것은, 너무 람다만 사용하면 가독성이 떨어질 수 있음
# 함수가 필요한 것은 함수로서 사용해 주는 것이 좋다.
# 람다식을 사용할 때는 익명함수를 사용할때 쓰는데,
# 람다식으로 바꿔보자.

lambda_mul_10 = lambda x: x * 10
# 첫 번째 x가 의미하는게 input, 출력은 x*10으로 나간다. 
# num이 x잖아요? 헷갈리니까 num으로 바꿔보자.
lambda_mul_10 = lambda num: num * 10
# 불필요한 이런 리턴문 다 없애버리고. num * 10을 리턴해라 이런 의미
# 실행하면?
print('>>>>', lambda_mul_10(10))

# 그래도 100이 출력된다. 
# 단순히 코드의 길이를 줄이는 것이 아니라, 가독성이나 코드를 간결하게 사용할 수 있다.
# 그렇다고 많은 메모리가 절약되는 것은 아니다.
# 람다식은 데이터 전처리나, 웹에서 사용할 게시판 데이터를 대량으로 가져와서 날짜를 바꿔준다던지, 내용을 수정한다던지, 형태소 분석을 하거나
# 문자열과 문자열을 연결해서 새로운 문자열을 만든다거나 할 때 좋다. 

# 마지막으로 lambda를 할당했기 때문에

def func_final(x, y, func):
    print( x * y * func(10))

# 매개변수로 함수를 받을 수도 있다. 
# 그랬을 때, print(x * y * func(10))
# 그러면 10* 10이니까 100
...
# 함수 자체를 즉시 만들어서 넣었다.
# 그래서 함수는 위로 넘어갔고 실행은 print에서 한 것
# 그래서 100만이 출력된 것을 확인할 수 있다. 
# 즉시 람다식을 바로 작성해서 함수에 인자로 함수를 넘긴 것을 확인할 수가 있다. 
# 함수의 선언
# 함수의 다양한 사용 방법
# 리턴도 리스트 형태나 다양한 반환값으로도 할 수 있고
# 튜플 형태의 매개변수
# 딕셔너리 형태의 매개변수
# 축약식 람다함수. 가독성이나 코드 감ㅅ
# 특히 함수를 매개변수로 넘길 때. 이 함수를 넘기기 위해서 구지 위에 있는 것을 가져다가 사용할 수도 있지만,
# 한 번 실행할 경우에는 바로 함수를 람다식으로 작성해서 넘기면 그만큼 메모리를 절약할 수 있는 형태가 된다.
