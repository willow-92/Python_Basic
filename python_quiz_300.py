...
# 생성자란, 해당 클래스로부터 객체를 생성할 때 자동으로 호출되는 함수. 
# 일반적으로 함수는 사용자가 명시적으로 호출을 해줘야 하는데, 자동으로 파이썬 인터프리터가 불러주는 값들이 몇 개 있다.
# 그 중에 하나가 생성자이다. 
# 클래스 안에는 함수가 있을 수 있는데, 이 함수의 기능이 "응애응애" 라는 것을 출력
# 그 다음에 Human 객체를 생성. 이걸 아름이라는 변수로 바인딩. 
# 이렇게 함녀 자동으로 __init__함수가 호출이 되는데, 객체가 생성될 떄, 객체를 생성한 다음에 자동으로 함수 이름이 __init__이면, 이 함수를 파이썬 인터프리터가 객체를 생성할 때 마다
# 자동으로 호출해준다. 그래서 얘를 생성자라고 부른다. 
# 실행을 하면 출력이 되는것이 확인 된다. 

# 먼저 클래스가 정의된다는 소리는 Human이라는 클래스가 어떤 클래스 공간을 가르킨다.
# 이 공간 안에는 딕셔너리가 있는데, __init__ 이라는 딕셔너리가 있고 이게 key, value로는 함수가 있는 형태.
# 함수 자체도 function 클래스라는 객체인데, init 함수가 응애응애를 바인딩하고 있다고 개념적으로 생각하면 된다. 
# 이 상태. 이게 이제 클래스가 정의 된것.
# areum = Human() -> 괄호가 있으면 객체를 생성해 냄. 
# 비어있는 붕어빵을 만들어 내는 것. 붕어빵 틀로부터 붕어빵을 만들어준다. 
# 프로그램에서는 고유의 메모리를 할당한다고 생각하면 된다.
# 이 메모리 할당을 했다면, 그 다음에는, 생성자가 있다면 생성자를 호출해 주는 것. __init__ 을 호출하고. 이 함수를 호출하니까 pirnt 코드가 수행되는데
# 이번에는 인자가 하나 있다. 
# 함수 호출할 때 생각해보면, self라는 변수가 생기고. 이 self라는 변수에 누가 넘어오느냐, 현재 이 생성자는 왜 호출이 되었나?
# 우리가 areum = Human()을 생성할 때, 이 휴먼 클래스로 부터 객체를 생성했다. 이 객체를 누가 생성했느냐? 아름이라는 변수가 생성된 객체를 바인딩 하고 있다. 
# 그러면 이 self 자리에 누가 넘어오냐하면, areum이 넘어온다. 이 생성된 객체를 바인딩 하게 된다. 
# 파이썬 클래스에서 self라는 것은 생성된 객체를 의미한다고 생각하면 된다. 
# slef 자리로 areum이 바인딩이 되고. print문이 출력이 된다. 
# __init__도 함수 호출일 뿐이기 때문에 함수가 끝났으면 리턴된다. 
# 리턴 되면서 셀프라는 변수가 바인딩 되는게 없어진다. 
# 아름이라는 변수로 객체를 잡아놓은 공간은 바인딩이 되어 있기 때문에 없어지지 않는다. 
# 오른쪽이 끝났고 왼쪽에 바인딩 한다.
# 어려면, 객체가 생성될 때 생성자가 호출된다. 이건 우리가 명시적으로 호출하지 않아도 자동으로 호출된다고 이해하면 됨. 
